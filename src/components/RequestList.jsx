...
const RequestList = ({ sellerId }) => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await axios.get(`https://shelf-x-mj39.vercel.app/requests/${sellerId}`, {
  ...
  const handleApproveRequest = async (bookId, sellerId, userId) => {
    try {
      ...
      await axios.put(`https://shelf-x-mj39.vercel.app/requests/${bookId}/approve`, { sellerId, userId, bookName: buyerDetails?.bookName , buyerEmail: buyerDetails?.email  });
  ...
  const handleRejectRequest = async (bookId, sellerId, userId) => {
    try {
      await axios.put(`https://shelf-x-mj39.vercel.app/requests/${bookId}/reject`, { sellerId, userId });
      setRequests((prevRequests) =>
        prevRequests.map((req) =>
